...
void writeSysInfoToConsole(SysInfo sys, WINDOW* sys_win){
    ...
    mvwprintw(sys_win,2,2,getCString(( "OS: " + sys.getOS_name())));
    mvwprintw(sys_win,3,2,getCString(( "Kernel version: " + sys.getKernelVersion())));
    mvwprintw(sys_win,4,2,getCString( "CPU: "));
    wattron(sys_win,COLOR_PAIR(1));
    wprintw(sys_win,getCString(Utility::getProgressBar(sys.getCPU_percent())));
    ...
    wattron(sys_win,COLOR_PAIR(1));
    wprintw(sys_win,getCString(Utility::getProgressBar(sys.getMemPercent())));
    wattroff(sys_win,COLOR_PAIR(1));
    ...
    mvwprintw(sys_win,13,2,getCString(( "Up Time: " + Utility::convertTime(sys.getUpTime()))));
    wrefresh(sys_win);
}
